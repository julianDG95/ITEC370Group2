...
/**
  * Backup table 'test_data' with the name of the date they were backed up
  */
function Backup_Table($conn){
	//save file
	$query = "SELECT * INTO OUTFILE 'C:/xampp/htdocs/backups/db-backup-".time()."-test_data.sql' FROM test_data";
	$valid = $conn->query($query);
    if (!$valid) {
        echo " Backup Failed.";
    } else {
...
function Restore_Table($name){
	DeleteData();
	$conn = ConnectToDB();
	$backup_file = "C:/xampp/htdocs/backups/$name"; 
	$query = "LOAD DATA INFILE '$backup_file' INTO TABLE `test_data`";
